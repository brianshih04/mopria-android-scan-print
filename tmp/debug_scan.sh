...
echo "=== LOGCAT ==="
adb logcat -d -v time 2>&1 | grep "MopriaScan\|MopriaAwait"

echo "=== File check ==="
LATEST=$(adb shell "run-as com.brianshih.mopria.android.scanprint ls -t files/scans/" 2>&1 | head -1 | tr -d '\r\n')
echo "Latest: $LATEST"
adb shell "run-as com.brianshih.mopria.android.scanprint cat files/scans/$LATEST" > "C:/Projects/mopria-android-scan-print/tmp/app_debug_scan.jpg" 2>/dev/null
python -c "
import struct
with open(r'C:\Projects\mopria-android-scan-print\tmp\app_debug_scan.jpg', 'rb') as f:
    data = f.read()
i = 0
while i < len(data) - 1:
    if data[i] == 0xFF and data[i+1] == 0xC0:
        ...
        print(f'Result: {w}x{h} ({len(data):,} bytes)')
        break
    i += 1
"
