...
現況同步：2026-09-16；目前工作分支是 `main`，OCR／Searchable PDF hardening 與 Brother eSCL Content-Type interoperability fix 已合併。
...
- 2026-09-16 的 `main` 本機 gate 為 180 JVM tests、lint、debug／release APK、AAB 與 zipalign 通過；最近一份完整 API 36 instrumentation 快照仍是 2026-08-11 的 28 tests。Linux `./gradlew` exit 127 已由 `.gitattributes`／wrapper line-ending 與 executable 修復，`main@68c2112` 的 GitHub Actions 已成功。
