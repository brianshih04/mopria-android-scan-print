...
| 列印 | 目前 Android `PrintManager`／`PrintDocumentAdapter`；直接 IPP client（`IppPrintClient`）為進行中 spike（協定層，尚未接線） |
...
- 自行實作直接 IPP／IPPS（進行中：`IppPrintClient` 協定層 spike 已完成，待接 discovery + `RealIntegrationProvider` + 列印 UI）。
